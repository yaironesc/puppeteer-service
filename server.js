...
app.post('/download-apk', async (req, res) => {
    ...
    // IMPORTANT: This requires Google account credentials
    // Legal warning: Only use with apps you own or have permission to redistribute
    if (!email || !password) {
        return res.status(400).json({ 
            success: false, 
            error: 'Google account credentials (email, password) are required for APK download' 
        });
    }
    ...
    try {
        ...
        const downloadDir = '/tmp/apk-downloads';
        if (!fs.existsSync(downloadDir)) {
            fs.mkdirSync(downloadDir, { recursive: true });
        }
        
        // Use apkeep to download APK
        // Note: apkeep requires authentication with Google account
        const apkPath = path.join(downloadDir, `${app_id}.apk`);
        
        // Run apkeep command
        // Format: apkeep -a <app_id> -u <email> -p <password> <output_dir>
        const command = `apkeep -a ${app_id} -u "${email}" -p "${password}" ${downloadDir}`;
        ...
        const apkFile = files.find(f => f.endsWith('.apk') && f.includes(app_id));
        
        if (!apkFile || !fs.existsSync(path.join(downloadDir, apkFile))) {
            return res.status(500).json({
                success: false,
                error: 'APK download failed. Check credentials and app ID.',
                details: stderr || stdout
            });
        }
        
        const fullApkPath = path.join(downloadDir, apkFile);
        const stats = fs.statSync(fullApkPath);
        
        // Return APK file info (you can modify this to return the file directly)
        res.json({
            success: true,
            app_id: app_id,
            filename: apkFile,
            size: stats.size,
            path: fullApkPath,
            message: 'APK downloaded successfully. Use /get-apk endpoint to download the file.'
        });
        ...
        res.status(500).json({
            success: false,
            error: error.message || 'Error downloading APK',
...
app.get('/get-apk/:app_id', (req, res) => {
    ...
    const fileStream = fs.createReadStream(fullApkPath);
    fileStream.pipe(res);
});
